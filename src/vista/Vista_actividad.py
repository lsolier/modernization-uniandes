...
class Vista_actividad(QWidget):
    ...
    def mostrar_gastos_por_actividad(self, actividad, gastos):
        ...
        """
        self.nombreActividad = actividad.nombreActividad
        self.titulo+=" "+actividad.nombreActividad
        self.setWindowTitle(self.titulo)
        ...
        for gasto in self.gastos:
            ...
            etiqueta_nombre = QLabel(gasto.viajero[0].nombre + " " + gasto.viajero[0].apellido)
            etiqueta_nombre.setWordWrap(True)
            self.distribuidor_actividades.addWidget(etiqueta_nombre, numero_fila, 0)

            etiqueta_fecha = QLabel(str(gasto.fechaGasto))
            ...
            etiqueta_valor = QLabel("${:,.2f}".format(gasto.valorGasto))
    ...
    def mostrar_reporte_comensacion(self):
        ...
        """    
        self.hide()
        self.interfaz.mostrar_reporte_compensacion(self.nombreActividad)
    ...
    def mostrar_reporte_gastos_por_viajero(self):
        ...
        """    
        self.hide()
        self.interfaz.mostrar_reporte_gastos_viajero(self.nombreActividad)
    
    def crear_gasto(self):
        """
        Esta función ejecuta el diálogo para crear un gasto
        """    
        dialogo = Dialogo_crear_gasto(self.interfaz.dar_viajeros(), None, self.nombreActividad)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.insertar_gasto(self.nombreActividad)
    ...
    def editar_gasto(self, indice_gasto):
        ...
        """    
        dialogo = Dialogo_crear_gasto(self.interfaz.dar_viajeros(), self.gastos[indice_gasto], self.nombreActividad)
        dialogo.exec_()
        if dialogo.resultado == 1:
            self.interfaz.editar_gasto(self.nombreActividad)
